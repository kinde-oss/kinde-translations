        ...
        if (typeof value === "string" && key !== contextKey) {
          ...
          try {
            ...
            if (translations.length > 0) {
              ...
              console.log(`Translated "${value}" to "${obj[key]}"`)
            }
          } catch (e) {
            console.error(
              `Error translating path ${fullPath}: ${
...
export async function applyJsonDifferencesToFile(
      ...
      if (typeof newValue === "string" && deeplClient) {
        if (newValue.trim().length === 0) {
          ...
          try {
            ...
            console.log(
              `  Original Value: "${newValue}" (Type: ${typeof newValue})`
            )
            console.log(`  Context: "${contextValue || "None"}"`)
            const translations = await deeplClient.translate({
              text: newValue,
            ...
            if (translations.length > 0) {
              ...
              console.log(`Translated updated value to "${newValue}"`)
            }
          } catch (e) {
            console.error(
              `Error translating updated value for ${change.path}: ${
                e instanceof Error ? e.message : String(e)
              }`
            )
          }
  ...
  for (const change of differences.added) {
    ...
    if (typeof addedValue === "string" && deeplClient) {
      if (addedValue.trim().length === 0) {
        ...
        try {
          console.log(`Attempting translation for added path: ${change.path}`)
          console.log(
            `  Original Value: "${addedValue}" (Type: ${typeof addedValue})`
          )
          console.log(`  Context: "${contextValue || "None"}"`)
          const translations = await deeplClient.translate({
            text: addedValue,
          ...
          if (translations.length > 0) {
            ...
            console.log(`Translated added value to "${addedValue}"`)
          }
        } catch (e) {
          console.error(
            `Error translating added value for ${change.path}: ${
              e instanceof Error ? e.message : String(e)
            }`
          )
        }
      ...
      console.log(`Translating nested object for added path: ${change.path}`)
  ...
  try {
    ...
    console.log(`Successfully applied differences to ${absolutePath}.`)
  } catch (error) {
    throw new Error(
      `Failed to write updated JSON to file ${absolutePath}: ${
        error instanceof Error ? error.message : String(error)
      }`
    )
  }
...
async function runSynchronizationAndTranslationExample() {
    ...
    console.log("--- Initial File States ---");
    console.log("Source.json:\n", fs.readFileSync(sourceFilePath, 'utf8'));
    console.log("Target.json (before sync):\n", fs.existsSync(targetFilePath) ? fs.readFileSync(targetFilePath, 'utf8') : 'Does not exist');
    ...
    try {
        ...
        console.log(fs.readFileSync(targetFilePath, 'utf8'));
    } catch (error) {
